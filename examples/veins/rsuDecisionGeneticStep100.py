...
class GA:
    ...
    def evaluate(self, start_geneinfo):
        ...
        for i in range(nlength):
            ...
            operationTime = self.cpu / (cpuTmp * self.allnum / geneinfo[i]) + \
                            self.rsuList[list(self.rsuList.keys())[i]]['wait']
            ...
            tmpWait[int(list(self.rsuWaits.keys()).index(list(self.rsuList.keys())[i]))] += (self.cpu * geneinfo[i]) / (
                        cpuTmp * self.allnum)
        for rsu, property in allserviceRoad.items():
            tmpPos = rsu.replace('(', '').replace(')', '')
            tmpPos = [int(tmpPos.split(',')[0]), int(tmpPos.split(',')[1])]
            serviceRate += calIntegralServiceRate(getRoadLength(property[1], self.net, self.boundaries), tmpPos) * \
                           geneinfo[property[0]] / self.allnum
    ...
    def crossoperate(self, offspring):
        ...
        crosslocation = random.randint(0, 99)
    ...
    def mutation(self, crossoff):
        ...
        mutelocation = random.randint(0, 99)
...
def rsuDecision(param: PythonParam):
    ...
    try:
        ...
        # print(all_list)
        para1 = [CXPB, MUTPB, NGEN, popsize, maxpiece, rsuList, cpu, mem, boundaries, serviceRoadList, etaRoad,
                 etaFinal,
                 rsuWaits,
        ...
        for k in range(len(bestallocation)):
            if bestallocation[k] != 0:
                ...
                for tmp in tmpLines:
                    if tmp[0] == tmpPos:
                        core = int(tmpcore)
                        tmpRelayTime = len(tmpRelay[1:]) * mem * float(bestallocation[k]) * 8 * 0.01 / 1024 / maxRate
                        ...
                        if tmp[core] == '*':
                            ...
                            tmp[core] += taskName + '(0(' + str(int(transTime + tmpRelayTime + simTime + 7)) + ';'
                        tmp[core + 5] = str(float(tmp[core + 5]) - round(mem * float(bestallocation[k]) / 100, 2))
                        tmp[core + 9] = str(
                            rsuList[midPos]['wait'] + transTime + tmpRelayTime + tmpOperationTime + simTime)
                        if core == 4:
                            tmp[core + 9] += '\n'
                        break
