    ...
    if (mobility->getRoadId() != lastroadID || startflag == 1){
        ...
        if (lastroadID.at(0) != ':'){
            ...
            if ((U_Random() > generateP || startflag == 1) && ifSend < maxTask && RSUPositions.size() > 0){
                ...
                pc->setModuleName("vehDecisionCache");
                PythonCommunication::PythonParam pp;
                ...
                if (name == "1"){
                    ...
                    double task_cpu = ppr->getDouble("cpu");
                    double task_mem = ppr->getDouble("mem");
                    int task_type = ppr->getInt("tasktype");
                    int sub_tasktype = ppr->getInt("subtasktype");
                    std::string possibleRSU = ppr->getString("possibleRSU");
                    task->setCPU(task_cpu);
                    task->setMem(task_mem);
                    task->setStorage(task->getMem());
                    task->setTaskType(task_type);
                    task->setTasksubType(sub_tasktype);
...
long TraCIDemo11p::getCarId() {
    return myId;
}

Coord TraCIDemo11p::getCurPosition() {
    return curPosition;
}

void TraCIDemo11p::onBroadcast(ReportMessage *rm) {
    ...
    //            scheduleAt(simTime() + uniform(0.01, 0.2), newRM->dup());
                ...
                Result<TraCIDemoRSU11p> result = RSUManager::getRsuInAera(curPosition, 1000);
                while (result.hasNext()) {
                    result.next()->onBroadcast(newRM);
                }
                ...
                findHost()->getDisplayString().setTagArg("t", 0, connectedRSUs.size());
}
